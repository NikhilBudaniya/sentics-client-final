  ...
  return (
    <div className="area-ctn">
            ...
            return (
              <Fragment>
                ...
                <div className="area-panel">
                  {!creatingArea ?
                    <button title="Create New Area" onClick={(e) => {
                      setCreatingArea(true);
                      document.querySelector(".area-name").style.display = "block";
                    }}><BiPlus /></button> :
                    <button title="Cancel Adding Area" onClick={(e) => setCreatingArea(false)}><MdOutlineCancel /></button>
                  }
                  <button title="Save Area Changes" onClick={saveArea}><BiSave /></button>
                </div>
      ...
      <div className="area-list">
        <div>
          <ul className="overflow-y-scroll max-h-[600px] list-style-none">
            {selectAreas.map((selectArea, index) =>
              <li key={index} className="bg-blue-50 rounded-md p-2 select-none my-2">
                <div className="font-bold text-xl">{selectArea.name}</div>
                ...
                <div className="flex">
                  <div className="mr-2 mt-2" onClick={() => deleteArea(index)}><BiTrash /></div>
...
function SelectArea({ selectArea, onChange, bgColor, display }) {
  ...
  useEffect(() => {
    // Using this rendering works perfect for react-moveable handles otherwise they are shown on top right position
    setTimeout(() => {
